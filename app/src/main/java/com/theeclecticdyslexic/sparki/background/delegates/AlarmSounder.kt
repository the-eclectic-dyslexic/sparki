...
/**
 * Handles sounding an alarm with optional vibration upon charge target being reached
 */

class AlarmSounder(context: Context) : BroadcastReceiver(), ChargeTargetReachedDelegate {
    ...
    private fun scheduleTimeout(context: Context) {
        val willTimeout = Settings.AlarmWillTimeout.retrieve(context)
        if (!willTimeout) return

        val timeout = Settings.AlarmTimeout.retrieve(context)
